...
const ProductFilters = () => {
    ...
    const [selectedCategories, setSelectedCategories] = useState<CategorySelectedInterface>({});
    ...
    const addCategory = (category: CategoryInterface) => {
        if (!selectedCategories[category.id]) {
            ...
            setSelectedCategories((current) => ({...current, [category.id]: category }));
        }
    };
    ...
    const getCategories = async () => {
        ...
        try {
            const res = await axios.get(`${import.meta.env.VITE_BASE_API_URL}/categories/products_count`);
                ...
                >
                    {categories.map((category) => (
                        <FormControlLabel
                            ...
                            control={
                                <Checkbox
                                    checked={selectedCategories[category.id] && category.id === selectedCategories[category.id].id}
                                    sx={{
                                        '&.Mui-checked': {
                                            color: '#70e000'
                                        }
